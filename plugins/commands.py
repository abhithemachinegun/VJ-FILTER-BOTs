...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if f_caption is None:
                f_caption = f"@atozmoviesflix {title}"
            try:
                if STREAM_MODE == True:
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"@atozmoviesflix {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            f_caption = f"@atozmoviesflix <code>{title}</code>"
            if CUSTOM_FILE_CAPTION:
                try:
